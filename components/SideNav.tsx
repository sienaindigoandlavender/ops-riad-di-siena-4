...
function NavLink({ href, label, icon, isActive }: { href: string; label: string; icon: React.ReactNode; isActive: boolean }) {
  return (
    <a
      href={href}
      title={label}
      className={`flex items-center gap-3 h-10 px-2.5 lg:px-3 transition-colors duration-150 relative group ${
        isActive
    ...
    >
      ...
      <span className="text-[11px] font-light tracking-[0.06em] hidden lg:block">{label}</span>
      ...
      <nav className="flex-1 py-2 px-2 lg:px-3">
        ...
        <div className="mt-4">
          <button
            ...
            className="flex items-center gap-2 px-2.5 lg:px-3 h-8 w-full text-left text-ink-tertiary hover:text-ink-secondary transition-colors"
          >
            <span className="text-[10px] font-light tracking-[0.1em] hidden lg:block">TOOLS</span>
            <svg className={`w-3 h-3 transition-transform hidden lg:block ${toolsOpen ? "" : "-rotate-90"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
            </svg>
          </button>
          {toolsOpen && (
            <div className="space-y-0.5 lg:pl-2">
          ...
          {allLinks.map(({ href, label }) => {
            ...
            return (
              <a
                key={href}
                href={href}
                className={`block py-3 text-[12px] tracking-[0.08em] transition-colors ${
                  isActive
